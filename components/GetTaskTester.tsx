...
function GetTaskTester() {
  const [taskId, setTaskId] = useState("");
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  const handleGetTask = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    setData(null);
    setError("");
  ...
  return (
    <div className="flex flex-col items-center p-6 bg-gray-100 min-h-screen">
      <h1 className="text-2xl font-bold mb-4">Test: /api/get-task Endpoint</h1>
      <form
        onSubmit={handleGetTask}
      ...
      >
        ...
        >
          {loading ? "Fetching Task..." : "Get Task"}
        </button>
      </form>
